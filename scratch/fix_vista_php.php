<?php
// Corrección de la vista v_movimientos_caja
$sql = "CREATE OR REPLACE VIEW bd_hospedajes.v_movimientos_caja AS
SELECT 
    'INGRESO' AS tipo,
...
try {
    $conexion = new PDO("mysql:host=localhost;dbname=bd_hospedajes", "root", "bd_hospedajes");
    $conexion->exec($sql);
    echo "<div class='alert alert-success'>Vista v_movimientos_caja actualizada correctamente con colación utf8mb4_unicode_ci</div>";
} catch (Exception $e) {
    echo "<div class='alert alert-danger'>Error al actualizar la vista: " . $e->getMessage() . "</div>";
}
?>
