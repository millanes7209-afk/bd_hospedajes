...
<body class="min-h-screen" style="background-color:var(--color-bg);color:var(--color-text);">
    ...
    <div class="max-w-7xl mx-auto px-4 py-8">
        <div class="flex items-center justify-between mb-6">
            <div class="flex items-center gap-2">
                <h2 class="text-2xl font-black uppercase admin-text-main"><i
                        class="fa-solid fa-clipboard-list mr-2 admin-text-gold"></i>PEDIDOS</h2>
                <span
                    class="text-xs font-bold bg-emerald-500/20 text-emerald-500 border border-emerald-500/40 px-3 py-1 rounded-full animate-pulse flex items-center gap-1.5 ml-2">
        ...
        @if (empty($pedidos))
            <div class="glass-card p-12 text-center">
                ...
                <h3 class="text-lg font-bold uppercase mb-2 admin-text-main">NO HAY PEDIDOS REGISTRADOS</h3>
                <p class="text-xs admin-text-muted">Las nuevas solicitudes realizadas por los clientes aparecerán aquí
                    automáticamente.</p>
            </div>
        @else
            <div class="space-y-4">
                @foreach ($pedidos as $pedido)
                    ...
                    $wspLink = "https://wa.me/591{$phoneClean}?text={$currWspMsg}";
                        ...
                        <div
                            class="glass-card p-5 border <?php        echo $est === 'pendiente' ? 'border-orange-500/80 bg-orange-500/10' : 'border-white/10'; ?>">
                                    ...
                                    <div class="flex items-center gap-3">
                                        ...
                                        <span
                                            class="px-3 py-1 rounded-full text-[11px] font-black uppercase tracking-wider shadow-sm
                                                                                                                                                                                                                                                                                                                                                   @if($est === 'pendiente') bg-orange-600 text-white border border-orange-700 animate-pulse font-black
                                                                                                                                                                                                                                                                                                                                                   @elseif($est === 'aceptado') bg-blue-500/20 text-blue-500 dark:text-blue-300 border border-blue-500/40
                                                                                                                                                                                                                                                                                                                                                   @elseif($est === 'preparando') bg-yellow-500/20 text-yellow-600 dark:text-yellow-300 border border-yellow-500/40
                                                                                                                                                                                                                                                                                                                                                   @elseif($est === 'listo') bg-emerald-500/20 text-emerald-600 dark:text-emerald-300 border border-emerald-500/40
                                                                                                                                                                                                                                                                                                                                                   @elseif($est === 'en_camino') bg-purple-500/20 text-purple-600 dark:text-purple-300 border border-purple-500/40
                                                                                                                                                                                                                                                                                                                                                   @elseif($est === 'entregado') bg-green-500/20 text-green-600 dark:text-green-300 border border-green-500/40
                                                                                                                                                                                                                                                                                                                                                   @else bg-red-500/20 text-red-600 dark:text-red-300 border border-red-500/40
                                                                                                                                                                                                                                                                                                                                                   @endif">
                                ...
                                <div class="flex items-center gap-2 flex-wrap">
                                    ...
                                    @if($est === 'pendiente')
                                        ...
                                        <form action="{{ route('admin.pedidos.aceptar', $pedido['pedidoID']) }}" method="POST"
                                            onsubmit="imprimirTicketSilencioso('{{ $pedido['pedidoID'] }}')" class="inline">
                                            @csrf
                                            <button type="submit"
                                                class="bg-emerald-600 hover:bg-emerald-500 text-white font-black text-xs py-1.5 px-3 rounded-lg shadow-md uppercase flex items-center gap-1">
                                                <i class="fa-solid fa-circle-check"></i>ACEPTAR E IMPRIMIR
                                            </button>
                                        </form>
                                    ...
                                    <!-- BOTÓN DE IMPRESIÓN DIRECTA SIN PESTAÑAS SECUNDARIAS -->
                                    <button type="button" onclick="imprimirTicketSilencioso('{{ $pedido['pedidoID'] }}')"
    ...
    <!-- SCRIPT AUTO-RELOAD EN TIEMPO REAL CON SONIDO E IMPRESIÓN DIRECTA -->
    <script>
        // IMPRESIÓN DIRECTA SILENCIOSA SIN ABRIR PESTAÑAS SECUNDARIAS
        function imprimirTicketSilencioso(pedidoId) {
            let iframe = document.getElementById('iframe-print-ticket');
            if (!iframe) {
                iframe = document.createElement('iframe');
                iframe.id = 'iframe-print-ticket';
            ...
            iframe.src = '{{ url("/ticket") }}/' + pedidoId + '?print=1';
        }
