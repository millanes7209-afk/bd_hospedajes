    ...
    <div class="glass-card p-6 md:p-8 rounded-2xl border border-white/10 shadow-2xl">
      <div class="flex items-center justify-between pb-4 border-b border-white/10 mb-6">
        ...
        <a href="{{ route('admin.productos') }}"
          class="text-xs admin-text-muted hover:admin-text-main uppercase font-bold">
      ...
      <?php if (!empty($error)): ?>
      <div
        class="mb-4 p-3 bg-red-500/20 border border-red-500/50 rounded-xl text-red-300 text-xs font-bold flex items-center gap-2">
        <i class="fa-solid fa-triangle-exclamation"></i>
        <span><?php  echo htmlspecialchars($error); ?></span>
      </div>
      <?php endif; ?>

      <form action="<?php echo $isEdit ? route('admin.productos.update', $id) : route('admin.productos.store'); ?>"
