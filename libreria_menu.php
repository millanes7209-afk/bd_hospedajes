...
// Consulta para verificar el estado de la caja abierta
...
<head>
    ...
    <style>
        @media (max-width: 991.98px) {
            .navbar-toggler { order: 1 !important; margin-left: 0 !important; margin-right: auto !important; }
            .navbar-brand { order: 2 !important; margin-left: auto !important; margin-right: 0 !important; }
            .navbar-collapse { order: 3 !important; width: 100% !important; }
        }
        @media (min-width: 992px) {
            #sideNav .navbar-toggler { display: block !important; cursor: pointer; }
            body:not(.menu-abierto) { padding-top: 60px !important; padding-left: 0 !important; transition: padding 0.3s ease; }
            body:not(.menu-abierto) #sideNav { flex-direction: row !important; width: 100vw !important; height: 60px !important; align-items: center; overflow: hidden; }
            body:not(.menu-abierto) #sideNav .navbar-toggler { order: 1 !important; margin-left: 0 !important; margin-right: auto !important; }
            body:not(.menu-abierto) #sideNav .navbar-brand { order: 2 !important; margin-left: auto !important; margin-right: 0 !important; }
            body:not(.menu-abierto) #sideNav .navbar-brand .d-none.d-lg-block { display: none !important; }
            body:not(.menu-abierto) #sideNav .navbar-brand .d-lg-none { display: block !important;  }
            body:not(.menu-abierto) #sideNav .navbar-collapse { display: none !important; }
            body.menu-abierto { padding-top: 0 !important; padding-left: 17rem !important; transition: padding 0.3s ease; }
            body.menu-abierto #sideNav { width: 17rem !important; height: 100vh !important; flex-direction: column !important; align-items: center; }
            body.menu-abierto #sideNav .navbar-toggler { position: absolute; top: 10px; left: 10px; z-index: 10; }
            body.menu-abierto #sideNav .navbar-collapse { display: flex !important; width: 100%; }
        }

        /* --- Estilos para la Leyenda de Puntos (Contextual) --- */
        .header-leyenda { display: flex; flex-direction: column; font-size: 10px; line-height: 1.2; justify-content: center; margin: 0 20px; border-left: 1px solid #ddd; padding-left: 15px; }
        .leyenda-item { display: flex; align-items: center; white-space: nowrap; margin-bottom: 1px; }
        .leyenda-dot { width: 10px; height: 10px; border-radius: 50%; margin-right: 6px; }
        .dot-ocupada { background-color: #007bff; }
        .dot-reservada { background-color: #17a2b8; }
        .dot-disponible { background-color: #28a745; }
        .dot-deuda { background-color: #dc3545; }
        .dot-limpieza { background-color: #6c757d; }
        .dot-mantenimiento { background-color: #343a40; }
        
        /* Botones de acción en header */
        .header-actions { display: flex; flex-direction: column; align-items: stretch; gap: 4px; margin-right: 15px; }
        .btn-header-acc { padding: 4px 10px; font-size: 11px; font-weight: bold; text-transform: uppercase; border: none; border-radius: 4px; color: white; cursor: pointer; }
        .btn-header-ingreso { background-color: #28a745; }
        .btn-header-egreso { background-color: #fd7e14; }
        .btn-header-acc:disabled { opacity: 0.5; cursor: not-allowed; }
    </style>
    <script>
            ...
            <div class='collapse navbar-collapse' id='navbarSupportedContent'>
                <ul class='navbar-nav'>
                    ...
                    <li class='nav-item mt-4 mb-3'>
                        <a class='nav-link text-warning' href='../../selector_empresa.php'>
                            <i class="fas fa-arrow-left"></i> VOLVER A EMPRESAS
                        </a>
                    </li>
                    <li class='nav-item'>
                        <a class='nav-link text-danger' href='<?= ($cuerp == false ? "../../validar.php" : "validar.php") ?>'>
            ...
            <div class='saldo-caja d-flex align-items-center' style='margin-left: auto;'>
                ...
                ?>
                    ...
                    <div class="header-actions">
                        <button type="button" class="btn-header-acc btn-header-ingreso" onclick="mostrarModalIngreso()" <?= $boton_header_estado ?>>Otros Ingresos</button>
                        <button type="button" class="btn-header-acc btn-header-egreso" onclick="mostrarModalEgreso()" <?= $boton_header_estado ?>>Registrar Egreso</button>
                    </div>
                <?php endif; ?>

                <?php if ($_SESSION["sesion_rol"] == 'RECEPCIONISTA' || $_SESSION["sesion_rol"] == 'ADMINISTRADOR'): ?>
                    <?php if ($caja_abierta): ?>
                        <button type='button' class='btn btn-danger' data-toggle='modal' data-target='#modalCerrarCaja' style='margin-right: 15px;'>
                            <i class='fas fa-lock'></i> Cerrar Caja
                        </button>
                    <?php else: ?>
                        <form action='<?php echo ($cuerp == 'false') ? 'procesar_caja.php' : '../../procesar_caja.php'; ?>' method='post' style='margin-right: 15px;'>
...
<div id="miModal" class="modal">
    <div class="modal-contenido">
        <div class="modal-header"><span class="cerrar">&times;</span><h2>Notificación</h2></div>
        <div class="modal-body"><p id="modalMensaje">Aquí irá el mensaje de la notificación.</p></div>
        <div class="modal-footer"><button id="facturaEmitidaBtn">Factura Emitida</button><button id="posponerBtn">Posponer</button></div>
    </div>
</div>

<?php 
if (isset($caja_abierta) && $caja_abierta && ($_SESSION["sesion_rol"] == 'RECEPCIONISTA' || $_SESSION["sesion_rol"] == 'ADMINISTRADOR')) {
    $saldos_modal = array_filter($saldos_forma_pago, function($saldo) { return $saldo > 0; });
    ...
    if ($grupo_actual_id) {
        ...
        foreach ($rs as $p) {
            if ($p['grupoID'] == $grupo_actual_id) {
                // Solo mostrar la pestaña activa si realmente coincide con el archivo actual
                $es_activa = (strpos($p['contenido'], $dir_actual) !== false) ? 'active' : '';
                
                // Usamos la ruta de la base de datos ajustando el prefijo tal como en el menú lateral
                $ruta_final = ($cuerp == false ? '../' : 'sis_segundo_2023/') . $p['contenido'];
                
                echo "<a href='$ruta_final' class='nav-tab-item $es_activa'>" . $p['opcion'] . "</a>";
            }
        }
        echo '</div>';
    }
