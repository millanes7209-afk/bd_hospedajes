...
if ($rol === 'RECEPCIONISTA') {
    ...
    // En caso de que no se haya filtrado por un usuario específico, mostrar todos los usuarios
    // No es necesario agregar una condición aquí porque la consulta por defecto traerá todos los usuarios
}

// Filtrar por tipo de movimiento si es necesario
if ($tipoReporte === 'ingreso' || $tipoReporte === 'egreso') {
    $query .= " AND tipo_movimiento = '" . strtoupper($tipoReporte) . "'"; // Cambié la forma en que se pasa el tipo
}
    ...
    case 'intervalo':
        if (isset($_GET['fechaInicio']) && isset($_GET['fechaFin'])) {
            ...
            if (!empty($fechaInicio) && !empty($fechaFin)) {
                ...
                if ($tipoReporte === 'ingreso' || $tipoReporte === 'egreso') {
                    $query .= " AND tipo_movimiento = '" . strtoupper($tipoReporte) . "'"; // Filtrado por tipo (ingreso o egreso)
                }
...
<head>
    ...
    .egreso {
        color:rgb(222, 17, 37); /* Texto rojo oscuro */
    }
</style>
    </style>
...
<!-- Botón para imprimir el reporte -->
    <button class="btn btn-primary mb-4" onclick="window.print()">Imprimir Reporte</button>
                    ...
                    <tbody>
                        <?php while (!$result->EOF): ?>
                            <tr>
                                ...
                                <!-- Modificar esta sección de la tabla donde se muestra la fecha -->
                                <td>
                                <a href="#" onclick="mostrarDetalles('<?php echo $result->fields['fecha']; ?>', '<?php echo $usuarioFiltradoID; ?>')">
                                        <?php echo date("d/m/Y", strtotime($result->fields['fecha'])); ?> 
                                        (<?php echo obtenerDiaEnEspanol($result->fields['fecha']); ?>)
                                    </a>
                                </td>
...
<script>
function mostrarDetalles(fecha, usuarioID) {
    var tipoReporte = '<?php echo $tipoReporte; ?>';  // Obtener el tipo de reporte actual (ingreso o egreso)

    var xhr = new XMLHttpRequest();
    xhr.open('GET', 'detalles.php?fecha=' + fecha + '&tipoReporte=' + tipoReporte + '&usuarioID=' + usuarioID, true);
    xhr.onload = function() {
        if (xhr.status == 200) {
            var detalles = JSON.parse(xhr.responseText);
            var detallesHTML = '<table class="table table-bordered"><thead><tr><th>Descripción</th><th>Monto</th><th>Fecha y Hora</th><th>Tipo</th><th>Forma Pago</th></tr></thead><tbody>';
            
            detalles.forEach(function(detalle) {
                var filaClass = (detalle.tipo.toUpperCase() === 'EGRESO') ? ' class="egreso"' : '';
                detallesHTML += '<tr' + filaClass + '><td>' + detalle.descripcion + '</td><td>' + detalle.monto + '</td><td>' + detalle.fecha_hora + '</td><td>' + detalle.tipo + '</td><td>' + detalle.forma_pago + '</td></tr>';
            });
...
<!-- Sección para mostrar los detalles -->
