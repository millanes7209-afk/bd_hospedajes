...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['accion'])) {
    ...
    if ($accion === 'guardar') {
        ...
        if ($tipohabitacionID) {
            $sql = "UPDATE tipo_habitaciones SET nombre = ?, precio = ?, descripcion = ?, _fec_modificacion = ?, _usuario = ? 
                    WHERE tipohabitacionID = ? AND empresaID = ?";
            $db->ejecutar($sql, [$nombre, $precio, $descripcion, $ahora, $usuarioID, $tipohabitacionID, $empresaID]);
        } else {
            $sql = "INSERT INTO tipo_habitaciones (nombre, precio, descripcion, empresaID, _fec_insercion, _usuario, _estado) 
                    VALUES (?, ?, ?, ?, ?, ?, 'A')";
            $db->ejecutar($sql, [$nombre, $precio, $descripcion, $empresaID, $ahora, $usuarioID]);
        }
    } elseif ($accion === 'eliminar') {
        $tipohabitacionID = $_POST['tipohabitacionID'];
        $sql = "UPDATE tipo_habitaciones SET _estado = 'X', _fec_modificacion = ?, _usuario = ? 
                WHERE tipohabitacionID = ? AND empresaID = ?";
        $db->ejecutar($sql, [$ahora, $usuarioID, $tipohabitacionID, $empresaID]);
    }
                            ...
                            <tbody>
                                <?php foreach ($tipos as $t): ?>
                                    <tr>
                                        ...
                                        <td><small><?= $t['descripcion'] ? htmlspecialchars($t['descripcion']) : '' ?></small>
                                        </td>
                                        <td class="text-end monto">Bs. <?= number_format($t['precio'], 2) ?></td>
                                        <td class="text-center">
                                            <div class="d-flex justify-content-center gap-4">
                                                <button
                                                    style="background:none; border:none; color:#17a2b8; padding:0; cursor:pointer;"
                                                    title="Modificar"
                                                ...
                                                <button
                                                    ...
                                                    title="Eliminar"
                                                    onclick="eliminarTipo(<?= $t['tipohabitacionID'] ?>, '<?= addslashes($t['nombre']) ?>')">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="nombre" id="txtNombre" class="form-control" required
                                placeholder="Ej: MATRIMONIAL" onkeyup="this.value = this.value.toUpperCase()">
                        </div>
                        <div class="mb-3">
                            <label class="form-label fw-bold">Precio Base (Bs.):</label>
                            <input type="number" step="0.50" name="precio" id="txtPrecio" class="form-control" required
                                placeholder="0.00">
                        </div>
                        <div class="mb-3">
                            <label class="form-label fw-bold">Descripción / Detalles:</label>
                            <textarea name="descripcion" id="txtDescripcion" class="form-control" rows="2"
