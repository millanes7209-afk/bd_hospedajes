...
resultadosFiltrados.forEach((fila) => {
    ...
    tbody.innerHTML += `
        <tr>
            <td>${highlight(fila.clienteID, ci)}</td>s
            ...
            <td>
                <form name="formModif${fila.clienteID}" method="post" action="cliente_modificar.php" style="display:inline;">
            ...
            <td>
                <button class="btn btn-sm btn-danger btn-accion eliminar-cliente" data-clienteid="${fila.clienteID}" data-clientes="${fila.clientes}">Eliminar</button>
            </td>
        </tr>
    `;
});
