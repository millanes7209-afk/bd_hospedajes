...
class ReporteController extends Controller
{
    public function index(Request $request)
    {
        ...
        $ventasCerradas = Venta::where('estado', 'cerrada')
        ...
        $ventasPorOrigen = Venta::where('estado', 'cerrada')
            ->whereDate('fecha_apertura', '>=', $fechaInicio)
            ->whereDate('fecha_apertura', '<=', $fechaFin)
        ...
        // 3. Productos Más Vendidos (Salteñas y Refrescos)
        $productosVendidos = VentaItem::whereHas('venta', function ($q) use ($fechaInicio, $fechaFin) {
        ...
        $ventasPorHora = Venta::where('estado', 'cerrada')
            ->whereDate('fecha_apertura', '>=', $fechaInicio)
            ->whereDate('fecha_apertura', '<=', $fechaFin)
